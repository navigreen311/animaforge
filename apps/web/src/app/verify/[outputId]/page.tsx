...
import { fetchVerification, isUnavailable, presentStatus } from '@/lib/governance/c2pa';
import type {
  ManifestAssertion,
  ServiceUnavailable,
  StatusPresentation,
...
function ClaimRow({
  ...
  const colour =
    state === 'yes' ? TONE.verified.fg : state === 'no' ? TONE.danger.fg : TONE.neutral.fg;
  return (
    <div
      style={{
        display: 'flex',
    ...
    >
      ...
      <span style={{ fontSize: 18, color: colour, flexShrink: 0 }} aria-label={state}>
        {glyph}
      </span>
    </div>
  );
...
function formatTime(iso: string | null): string {
  ...
  return Number.isNaN(date.getTime()) ? iso : `${date.toUTCString().replace('GMT', 'UTC')}`;
}
...
function ResultView({ result, outputId }: { result: VerificationResult; outputId: string }) {
  ...
  return (
    <div>
      ...
      {result.reason && result.status !== 'valid' && (
        ...
        >
          <h3 style={{ ...sectionTitle, color: TONE[presentation.tone].fg }}>Why</h3>
          <p
            style={{
              margin: 0,
              fontSize: 13,
      ...
      {result.signature && (
        <div style={card}>
          ...
          >
            <Field label="Algorithm" value={result.signature.algorithm ?? '—'} />
            ...
            <Field
              ...
              monospace
            />
            <Field label="Timestamp (RFC 3161)" value={formatTime(result.signature.timestamp)} />
          </div>
        </div>
      )}
      ...
      <div style={card}>
        ...
        >
          ...
          {generation?.watermark_id ? (
            <Field label="Watermark ID" value={String(generation.watermark_id)} monospace />
          ) : null}
        </div>
      </div>
          ...
          >
            {result.validation_status.map((entry, i) => (
              <li key={`${entry.code ?? 'entry'}-${i}`}>
                <span style={{ fontFamily: mono }}>{entry.code ?? 'status'}</span>
      ...
      >
        A lookup by output ID can only confirm what AnimaForge recorded. To validate a specific
        file, submit the file itself to{' '}
        <code style={{ fontFamily: mono }}>POST /governance/c2pa/verify</code> — that is the only
        check that can return &ldquo;cryptographically verified&rdquo;.
      </p>
    </div>
  );
}
    ...
    >
      <div style={{ maxWidth: 620, width: '100%' }}>
        ...
        {loading ? (
          ...
          <Notice tone="neutral" headline="Nothing to show" detail="No identifier was supplied." />
        )}
