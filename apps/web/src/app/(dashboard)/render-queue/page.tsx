...
function TierBadge({ tier }: { tier: RenderTier }) {
  const cfg: Record<RenderTier, { bg: string; text: string; border: string }> = {
    ...
    priority: {
      ...
      text: 'var(--brand-light, #a78bfa)',
      border: 'var(--brand-border, rgba(124,58,237,0.35))',
    },
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div
        style={{ fontSize: 20, fontWeight: 700, color: accent || 'var(--text-primary, #e2e8f0)' }}
      >
        {value}
      </div>
    </div>
...
export default function RenderQueuePage() {
  ...
  const [sortCol, setSortCol] = useState<'project' | 'shot' | 'duration' | 'credits' | 'completed'>(
    'completed',
  );
  ...
  const successRate =
    todayJobs > 0 ? Math.round((completeCount / (completeCount + failedCount)) * 100) : 0;
    ...
    cj.sort((a, b) => {
      ...
      switch (sortCol) {
        case 'project':
          ...
          break;
        case 'shot':
          cmp = a.shotLabel.localeCompare(b.shotLabel);
          break;
        case 'duration':
  ...
  const pagedComplete = completeJobs.slice(
    completePage * COMPLETE_PER_PAGE,
    (completePage + 1) * COMPLETE_PER_PAGE,
  );
    ...
    <div style={{ padding: '20px 24px', fontFamily: 'var(--font-sans, system-ui)' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <StatCard
          label="Failed"
          value={failedCount}
          accent={failedCount > 0 ? '#f87171' : undefined}
        />
        ...
        {tabs.map((t) => (
          <button
            ...
            style={{
              ...
              border: 'none',
              borderBottom:
                activeTab === t.key ? '2px solid var(--brand, #7c3aed)' : '2px solid transparent',
              cursor: 'pointer',
              marginBottom: -1,
            }}
          >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 6,
                      borderRadius: 3,
                      background: 'var(--progress-track, rgba(255,255,255,0.07))',
                      overflow: 'hidden',
                    }}
                  >
                    <div
              ...
              <tbody>
                {pagedComplete.map((job) => (
                  <tr key={job.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '10px 14px', color: 'var(--brand-light, #a78bfa)' }}>
                      {job.credits}
                    </td>
                    <td
                      style={{ padding: '10px 14px', color: 'var(--text-tertiary)', fontSize: 12 }}
                    >
                      {job.completedAt
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span
                      style={{
                        fontSize: 14,
                        color: 'var(--text-tertiary)',
                        transform: expandedFailed.has(job.id) ? 'rotate(180deg)' : 'none',
