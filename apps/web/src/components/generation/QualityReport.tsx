...
/** Scores above threshold are "good" (green), below are "bad" (red). */
const THRESHOLDS: Record<keyof QualityScores, { good: number; invert: boolean }> = {
  flicker: { good: 0.8, invert: true },        // lower is better
  identityDrift: { good: 0.85, invert: true },  // lower is better
  loudness: { good: 0.7, invert: false },       // higher is better
  artifacts: { good: 0.8, invert: true },       // lower is better
};
        ...
        {entries.map(({ key, label, value, good }) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`min-w-[3ch] text-right text-xs font-semibold ${
