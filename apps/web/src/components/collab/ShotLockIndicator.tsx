...
export default function ShotLockIndicator({
  ...
  useEffect(() => {
    ...
    const update = () => { setTimeRemaining(Math.max(0, lockInfo.expiresAt! - Date.now())); };
    update();
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col items-center justify-center rounded-lg bg-black/50 backdrop-blur-[2px]">
      ...
      <span className="text-[10px] font-medium text-amber-300">{lockedByName || 'Another user'} is editing</span>
      {secondsLeft > 0 && <span className="text-[9px] text-gray-400 mt-0.5">Expires in {secondsLeft}s</span>}
      {onRequestUnlock && (
