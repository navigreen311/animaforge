    ...
    <tr className="border-b border-gray-800 hover:bg-gray-800/30 transition-colors">
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          <code className="text-sm text-gray-400 font-mono">{revealed ? keyValue : maskedKey}</code>
          <button
            type="button"
      ...
      <td className="px-4 py-3">
        <div className="flex flex-wrap gap-1">
          {scopes.map((scope) => (
            <span
              key={scope}
              className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-violet-900/30 text-violet-300 border border-violet-700/50"
            >
              {scope}
            </span>
          ))}
