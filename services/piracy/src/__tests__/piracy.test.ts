...
describe('Piracy Monitoring Service', () => {
  ...
  test('GET /piracy/dashboard — returns monitoring stats', async () => {
    await request(app).post('/piracy/register').send({ outputId: 'out-1', watermarkId: 'wm-1' });
  ...
  test('PUT /piracy/alerts/:id/action — rejects invalid action', async () => {
    const res = await request(app).put('/piracy/alerts/fake-id/action').send({ action: 'invalid' });
