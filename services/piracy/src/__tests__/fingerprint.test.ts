...
async function makeImage(variant = 0): Promise<Jimp> {
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      const i = (y * W + x) * 4;
      image.bitmap.data[i] =
        (Math.sin((x + variant * 91) / 31) * 100 + 120) | 0;
      image.bitmap.data[i + 1] =
        (Math.cos((y + variant * 57) / 19) * 80 + 130) | 0;
      image.bitmap.data[i + 2] =
        ((((x * 3) ^ (y * 5)) + variant * 40) % 220) + 20;
...
describe("Perceptual hashing", () => {
  it("is stable across a lossy re-encode", async () => {
    ...
    for (const quality of [80, 40, 15]) {
      ...
      const distance = hammingDistance(
        base.phash,
        (await hashImageBuffer(reencoded)).phash,
      );
      expect(
        distance,
        `pHash drifted too far at quality ${quality}`,
  ...
  it("is stable across a rescale", async () => {
    ...
    const distance = hammingDistance(
      base.phash,
      (await hashImageBuffer(halved)).phash,
    );
  ...
  it("is not an exact-hash lookup — identical content, different bytes", async () => {
    ...
    expect(hammingDistance(a.phash, b.phash)).toBeLessThanOrEqual(
      MATCH_THRESHOLD,
    );
  }, 120_000);
...
describe("Fingerprint registration and matching", () => {
  ...
  it("refuses to fingerprint when no media is supplied", async () => {
    await expect(computeFingerprint({})).rejects.toThrow(
      /asset_base64 or asset_path/,
    );
  });
});
...
describe("Piracy — fingerprint HTTP API", () => {
  it("registers with a fingerprint and matches a re-encoded copy", async () => {
    ...
    expect(
      matchRes.body.matches[0].hamming_distance ??
        matchRes.body.matches[0].distance,
    ).toBeLessThanOrEqual(MATCH_THRESHOLD);
  }, 180_000);
