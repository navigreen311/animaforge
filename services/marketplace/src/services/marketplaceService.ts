...
function mapCategoryFromType(type: string): string {
  return type;
}
    ...
    try {
      ...
      if (dbItem) {
        return {
          ...
          createdAt:
            dbItem.createdAt instanceof Date
              ? dbItem.createdAt.toISOString()
              : String(dbItem.createdAt),
...
export async function searchItems(query: z.infer<typeof SearchSchema>): Promise<{
  ...
  return {
    items: results.slice(start, start + query.limit),
    total,
    page: query.page,
    limit: query.limit,
  };
...
export function submitReview(
  ...
  const itemReviews = reviews.get(itemId) ?? [];
  if (itemReviews.some((r) => r.userId === data.userId))
    return { error: 'User has already reviewed this item' };
  const review: Review = {
    id: uuidv4(),
    itemId,
    userId: data.userId,
...
export function getSellerAnalytics(creatorId: string): SellerAnalytics {
  ...
  for (const item of creatorItems) {
    for (const r of reviews.get(item.id) ?? []) {
  ...
  const topItems = creatorItems
    .filter((i) => itemRevenue.has(i.id))
    .map((i) => ({
      id: i.id,
      name: i.name,
      sales: itemSales.get(i.id) ?? 0,
      revenue: +(itemRevenue.get(i.id) ?? 0).toFixed(2),
    }))
...
export function rateItem(
  ...
  const itemReviews = reviews.get(itemId) ?? [];
  if (itemReviews.find((r) => r.userId === userId))
