...
describe('Plugin Certification System', () => {
  ...
  it('POST /api/v1/plugins — registers a plugin with pending_review status', async () => {
    const res = await request(app).post('/api/v1/plugins').set(USER_AUTH).send(VALID_MANIFEST);
  ...
  it('GET /api/v1/plugins — lists plugins, supports category filter', async () => {
    ...
    const res = await request(app).get('/api/v1/plugins').set(USER_AUTH);

    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(2);

    const filtered = await request(app).get('/api/v1/plugins?category=export').set(USER_AUTH);
  ...
  it('GET /api/v1/plugins/:id — returns plugin details', async () => {
    ...
    const res = await request(app).get(`/api/v1/plugins/${reg.pluginId}`).set(USER_AUTH);
  ...
  it("GET /api/v1/plugins/installed — returns user's installed plugins", async () => {
    ...
    const res = await request(app).get('/api/v1/plugins/installed').set(USER_AUTH);
  ...
  it('GET /api/v1/plugins/:id/metrics — returns plugin metrics', async () => {
    ...
    const res = await request(app).get(`/api/v1/plugins/${reg.pluginId}/metrics`).set(USER_AUTH);
