...
describe('Human Review Service (N6)', () => {
  ...
  it('GET /api/v1/review-queue — returns filtered queue', async () => {
    ...
    const res = await request(app).get('/api/v1/review-queue?type=content_moderation').set(AUTH);

    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(2);
    expect(res.body.data.every((t: { type: string }) => t.type === 'content_moderation')).toBe(
      true,
    );
  });
  ...
  it('GET /api/v1/review-queue/metrics — returns queue metrics', async () => {
    ...
    const res = await request(app).get('/api/v1/review-queue/metrics').set(AUTH);
  ...
  it('POST /api/v1/review-queue/auto-assign — assigns pending tasks round-robin', async () => {
    ...
    const res = await request(app).post('/api/v1/review-queue/auto-assign').set(AUTH);
  ...
  it('POST /api/v1/support/tickets — creates a support ticket', async () => {
    const res = await request(app).post('/api/v1/support/tickets').set(AUTH).send({
      userId: '00000000-0000-4000-8000-000000000001',
      subject: 'Cannot export',
      body: 'Export fails on MP4',
      category: 'technical',
    });
  ...
  it('POST /api/v1/support/tickets/:id/respond — adds response to ticket', async () => {
    const ticket = humanReviewService.createSupportTicket(
  ...
  it('PUT /api/v1/support/tickets/:id/close — closes the ticket', async () => {
    ...
    const res = await request(app).put(`/api/v1/support/tickets/${ticket.id}/close`).set(AUTH);
