...
describe('Social Publishing (F4)', () => {
  ...
  it('GET /api/v1/social/connections — lists connected platforms', async () => {
    socialService.connectPlatform('00000000-0000-4000-8000-000000000001', 'youtube', {
      accessToken: 'yt',
    });
    socialService.connectPlatform('00000000-0000-4000-8000-000000000001', 'tiktok', {
      accessToken: 'tt',
    });

    const res = await request(app).get('/api/v1/social/connections').set(AUTH);

    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(2);
    expect(res.body.data.map((c: { platform: string }) => c.platform).sort()).toEqual([
      'tiktok',
      'youtube',
    ]);
  });
  ...
  it('DELETE /api/v1/social/disconnect — disconnects a platform', async () => {
    socialService.connectPlatform('00000000-0000-4000-8000-000000000001', 'instagram', {
      accessToken: 'ig',
    });
  ...
  it('GET /api/v1/social/history — returns publication history', async () => {
    socialService.publishToYouTube(
      '00000000-0000-4000-8000-000000000001',
      'https://cdn.animaforge.io/v1.mp4',
      { title: 'V1' },
    );
    socialService.publishToTikTok(
      '00000000-0000-4000-8000-000000000001',
      'https://cdn.animaforge.io/v2.mp4',
      { title: 'V2' },
    );

    const res = await request(app).get('/api/v1/social/history').set(AUTH);
