...
describe('UX Receipts (D9)', () => {
  ...
  it('POST /api/v1/receipts — creates a receipt', async () => {
    const res = await request(app)
      ...
      .send({
        ...
        details: { projectId: PROJECT_ID, credits: 5 },
      });
  ...
  it('GET /api/v1/receipts/:id — returns a single receipt', async () => {
    ...
    const res = await request(app).get(`/api/v1/receipts/${receipt.receiptId}`).set(AUTH);
  ...
  it('GET /api/v1/receipts/project/:projectId — returns project receipts', async () => {
    ...
    await receiptService.createReceipt(
      '00000000-0000-4000-8000-000000000001',
      'generation_completed',
      { projectId: '11111111-1111-4111-8111-000000000002' },
    );

    const res = await request(app).get(`/api/v1/receipts/project/${PROJECT_ID}`).set(AUTH);
