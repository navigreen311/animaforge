...
describe('Review Workflow', () => {
  ...
  it('POST /api/v1/reviews/:id/comments — adds a comment', async () => {
    ...
    const res = await request(app).post(`/api/v1/reviews/${review.id}/comments`).set(AUTH).send({
  ...
  it('POST /api/v1/reviews/:id/comments — supports threading via parentId', async () => {
    ...
    const parent = await reviewService.addComment(
      review.id,
  ...
  it('GET /api/v1/reviews/shot/:shotId — returns review history ordered by round', async () => {
    ...
    const res = await request(app).get(`/api/v1/reviews/shot/${SHOT_ID}`).set(AUTH);
  ...
  it('PUT /api/v1/reviews/:id/escalate — escalates from editor to director', async () => {
    ...
    const res = await request(app).put(`/api/v1/reviews/${review.id}/escalate`).set(AUTH);
    ...
    await reviewService.submitReview(review.id, 'approved');
    const res2 = await request(app).put(`/api/v1/reviews/${review.id}/escalate`).set(AUTH);
  ...
  it('PUT /api/v1/comments/:id/resolve — resolves a comment', async () => {
    ...
    const res = await request(app).put(`/api/v1/comments/${comment!.id}/resolve`).set(AUTH);
  ...
  it('GET /api/v1/reviews/project/:projectId — returns all reviews for a project', async () => {
    ...
    const res = await request(app).get(`/api/v1/reviews/project/${PROJECT_ID}`).set(AUTH);
