...
const SAMPLE_BRAND_KIT = {
  ...
  logo: {
    url: 'https://cdn.animaforge.io/logos/logo.png',
    placement: 'top-left' as const,
    minSize: 48,
  },
  typography: { headingFont: 'Inter', bodyFont: 'Roboto', sizes: [{ label: 'H1', value: 32 }] },
  sonic: {
    introUrl: 'https://cdn.animaforge.io/audio/intro.mp3',
    outroUrl: 'https://cdn.animaforge.io/audio/outro.mp3',
    transitionUrl: '',
  },
...
describe('Brand Kit + Social Publishing (F4)', () => {
  ...
  it('POST /api/v1/social/publish-with-brand — publishes with brand kit applied', async () => {
    const res = await request(app).post('/api/v1/social/publish-with-brand').set(AUTH).send({
      platform: 'youtube',
  ...
  it('POST /api/v1/social/schedule-multi — schedules to multiple platforms', async () => {
    ...
    expect(res.body.data.publications.map((p: { platform: string }) => p.platform).sort()).toEqual([
      'instagram',
      'tiktok',
      'youtube',
    ]);
  ...
  it('GET /api/v1/social/calendar — returns publishing calendar for a month', async () => {
    ...
    const res = await request(app).get('/api/v1/social/calendar').set(AUTH).query({ month });
  ...
  it('GET /api/v1/social/performance/:publishId — returns performance metrics', async () => {
    const pubRes = await request(app).post('/api/v1/social/publish-with-brand').set(AUTH).send({
      platform: 'tiktok',
  ...
  it('GET /api/v1/social/performance/:publishId — returns 404 for unknown id', async () => {
    const res = await request(app).get('/api/v1/social/performance/nonexistent-id').set(AUTH);
  ...
  it('GET /api/v1/social/cross-platform-report — aggregates metrics across platforms', async () => {
    socialService.publishToPlatform(
      '00000000-0000-4000-8000-000000000001',
      'youtube',
      'https://cdn.animaforge.io/v1.mp4',
      {},
    );
    socialService.publishToPlatform(
      '00000000-0000-4000-8000-000000000001',
      'tiktok',
      'https://cdn.animaforge.io/v2.mp4',
      {},
    );
  ...
  it('DELETE /api/v1/social/disconnect — revokes platform access', async () => {
    socialService.connectPlatform('00000000-0000-4000-8000-000000000001', 'vimeo', {
  ...
  it('GET /api/v1/social/platforms — returns supported platforms with specs', async () => {
    const res = await request(app).get('/api/v1/social/platforms').set(AUTH);
