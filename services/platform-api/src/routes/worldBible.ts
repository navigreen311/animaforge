...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const result = await wbs.enforceConstraints(req.params.projectId, req.body);
    res.status(200).json({ success: true, data: result });
  }),
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const result = await wbs.generateFromDescription(req.params.projectId, req.body.description);
    res.status(200).json({ success: true, data: result });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const profile = await wbs.getCharacterProfile(req.params.projectId, req.params.name);
    if (!profile) {
      res.status(404).json({
        success: false,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const details = await wbs.getLocationDetails(req.params.projectId, req.params.name);
    if (!details) {
      res.status(404).json({
        success: false,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const result = await wbs.checkSceneAgainstBible(req.params.projectId, req.body);
    res.status(200).json({ success: true, data: result });
  }),
);
