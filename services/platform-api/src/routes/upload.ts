...
router.post('/upload', (req: Request, res: Response): void => {
  ...
  if (!fileType) {
    res.status(400).json({
      ...
      error: {
        ...
        message: 'File type not supported. Accepted: image, video, audio, 3d-model',
      },
    });
    return;
  }
  ...
  const mimeType =
    contentType ??
    `${fileType === '3d-model' ? 'application' : fileType}/${filename.split('.').pop()}`;

  res.status(201).json({
    success: true,
    data: {
      url: `https://cdn.animaforge.ai/${key}`,
      key,
      type: fileType,
      size: fileSize,
      mimeType,
  ...
  if (!filename || !contentType || !projectId) {
    res.status(400).json({
      ...
      error: {
        ...
        message: 'filename, contentType, and projectId are required',
      },
    });
    return;
  }
