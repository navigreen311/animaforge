...
router.post('/social/publish-with-brand', requireAuth, async (req: Request, res: Response) => {
  ...
  if (!platform || !videoUrl || !brandKitId) {
    res.status(400).json({
      ...
      error: {
        ...
        message: 'platform, videoUrl, and brandKitId are required',
      },
    });
    return;
  }
  ...
  try {
    const result = await socialPublisher.publishWithBrand(
      req.user!.id,
      platform,
      videoUrl,
      brandKitId,
...
router.post('/social/schedule-multi', requireAuth, async (req: Request, res: Response) => {
  ...
  if (
    ...
    res.status(400).json({
      ...
      error: {
        code: 'VALIDATION_ERROR',
        message: 'videoUrl, platforms[], and scheduleAt are required',
      },
    });
    return;
  }
...
router.get('/social/calendar', requireAuth, (req: Request, res: Response) => {
  ...
  if (!month) {
    res.status(400).json({
      ...
      error: {
        code: 'VALIDATION_ERROR',
        message: 'month query parameter is required (e.g. 2026-03)',
      },
    });
    return;
  }
...
router.get('/social/cross-platform-report', requireAuth, (req: Request, res: Response) => {
  ...
  if (!period) {
    res.status(400).json({
      ...
      error: {
        code: 'VALIDATION_ERROR',
        message: 'period query parameter is required (e.g. 2026-03)',
      },
    });
    return;
  }
