...
export function invalidateCache(distributionId: string, paths: string[]): Invalidation {
  const dist = distributions.get(distributionId);
  if (!dist) {
    const err: Error & { statusCode?: number; code?: string } = new Error('Distribution not found');
    err.statusCode = 404;
    err.code = 'NOT_FOUND';
    throw err;
  }
...
export function estimateDeliveryCost(
  ...
  const rate = ratePerGb[region] ?? 0.1;
