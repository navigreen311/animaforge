...
export interface ValidationResult {
  valid: boolean;
  missing_consents: Array<{ character_ref: string; consent_type: string }>;
}
...
function isExpired(consent: { expires_at: string | null | Date }): boolean {
  ...
  const expiresAt =
    consent.expires_at instanceof Date ? consent.expires_at : new Date(consent.expires_at);
...
export async function grantConsent(
  ...
  try {
    if (await isDatabaseReachable()) {
      const row = await requirePrisma().consent.create({
        data: {
      ...
      return { consent_id: row.id, status: 'active' };
    }
  } catch {
    /* fall through */
  }
  const consent_id = uuidv4();
  consentStore.set(consent_id, {
    consent_id,
    subject_id,
    granted_by,
    consent_type,
...
export async function getConsentsBySubject(subject_id: string): Promise<ConsentRecord[]> {
  try {
    if (await isDatabaseReachable()) {
      const rows = await requirePrisma().consent.findMany({
        where: { subjectId: subject_id, revokedAt: null },
  ...
  for (const record of consentStore.values()) {
    if (record.subject_id === subject_id) records.push(record);
  }
  return records;
}
...
export async function revokeConsent(consent_id: string): Promise<ConsentRecord | null> {
  try {
    if (await isDatabaseReachable()) {
      const row = await requirePrisma().consent.update({
        where: { id: consent_id },
...
export async function validateConsents(
  ...
  for (const ref of character_refs) {
    ...
    for (const consentType of consent_types_needed) {
      if (!activeConsents.some((c) => c.consent_type === consentType)) {
