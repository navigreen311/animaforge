...
export async function startTestPki(): Promise<TestPki> {
  ...
  ssl(['ecparam', '-name', 'prime256v1', '-genkey', '-noout', '-out', 'leaf.sec1.key']);
  ssl(['pkcs8', '-topk8', '-nocrypt', '-in', 'leaf.sec1.key', '-out', 'leaf.key']);
  ssl([
    'req',
    '-new',
    '-key',
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      try {
        ...
        ssl(['ts', '-reply', '-config', 'tsa.cnf', '-queryfile', `${id}.tsq`, '-out', `${id}.tsr`]);
