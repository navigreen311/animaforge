...
describe('POST /governance/moderate/pre-check', () => {
  ...
  it('returns 400 for missing prompt', async () => {
    const res = await request(app).post('/governance/moderate/pre-check').send({});
...
describe('GET /governance/moderation-log/:jobId', () => {
  it('returns empty array for unknown job', async () => {
    const res = await request(app).get('/governance/moderation-log/nonexistent');
  ...
  it('returns log records after moderation', async () => {
    ...
    const res = await request(app).get('/governance/moderation-log/job-log-1');
