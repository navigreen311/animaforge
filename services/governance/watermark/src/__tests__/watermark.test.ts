...
import { DEFAULT_STRENGTH, embedIntoImage, extractFromImage } from '../lib/imageWatermark';
...
async function sourcePng(width = 512, height = 512): Promise<Buffer> {
  return Buffer.from(await (await makeSource(width, height)).getBuffer('image/png'));
}
...
describe('Watermark — payload framing', () => {
  ...
  it('rejects random bits — this is what stops false positives', () => {
    ...
    for (let trial = 0; trial < 500; trial++) {
      const bits = Array.from({ length: 64 }, () => (trial * 7 + Math.random() > 0.5 ? 1 : 0));
  ...
  it('computes a stable CRC', () => {
    expect(crc16([1, 0, 1, 0, 1, 0, 1, 0])).toBe(crc16([1, 0, 1, 0, 1, 0, 1, 0]));
    expect(crc16([1, 1, 1, 1, 0, 0, 0, 0])).not.toBe(crc16([0, 0, 0, 0, 1, 1, 1, 1]));
  });
});
...
describe('Watermark — pixel-domain embedding', () => {
  ...
  it('recovers the payload from the lossless carrier', async () => {
    const marked = await embedIntoImage(await sourcePng(), 'abcdef012345', SEED);
  ...
  it('recovers the payload after the image is re-encoded at lower quality', async () => {
    const marked = await embedIntoImage(await sourcePng(), '0f1e2d3c4b5a', SEED);
    ...
    for (const quality of [80, 65, 50, 40]) {
      ...
      expect(found.keyHex, `wrong key at JPEG quality ${quality}`).toBe('0f1e2d3c4b5a');
    }
  }, 180_000);

  it('survives a double re-encode', async () => {
    const marked = await embedIntoImage(await sourcePng(), '112233445566', SEED);
  ...
  it('finds nothing when read with the wrong seed', async () => {
    const marked = await embedIntoImage(await sourcePng(), 'aaaabbbbcccc', SEED);
  ...
  it('refuses to mark an image too small to carry the payload', async () => {
    await expect(embedIntoImage(await sourcePng(64, 64), 'abcdef012345', SEED)).rejects.toThrow(
      /carrier blocks per payload bit/,
    );
  }, 60_000);
...
describe('Watermark — HTTP API', () => {
  ...
  it('does not claim detection from a URL alone', async () => {
    const embedRes = await request(app).post('/governance/watermark/embed').send({
      job_id: 'job-003',
  ...
  it('rejects invalid input', async () => {
    expect((await request(app).post('/governance/watermark/embed').send({})).status).toBe(400);
    expect((await request(app).post('/governance/watermark/detect').send({})).status).toBe(400);
  });
  ...
  describe('Watermark — video path (ffmpeg)', () => {
    it('embeds into every frame and recovers after H.264 re-encode', async () => {
      ...
      try {
        ...
        const result = await embedIntoVideo(source, marked, 'beefcafe1234', SEED);
