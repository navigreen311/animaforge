...
import { calculateProgress, StageDefinition, JobStatus } from '../utils/jobHelpers.js';
...
vi.mock('bullmq', () => {
  ...
  const MockWorker = vi.fn().mockImplementation((_name: string, processor: unknown) => {
...
describe('Generation worker', () => {
  ...
  it('creates a worker that registers event handlers', async () => {
    const { createGenerationWorker } = await import('../workers/generationWorker.js');
...
describe('Governance worker', () => {
  it('creates a worker with event handlers', async () => {
    vi.clearAllMocks();
    const { createGovernanceWorker } = await import('../workers/governanceWorker.js');
