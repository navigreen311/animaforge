...
async function cleanExpiredJobs(): Promise<number> {
  ...
  if (usePrisma) {
    try {
      return (
        await requirePrisma().generationJob.deleteMany({
          where: { status: { in: ['failed', 'complete'] }, completedAt: { lt: cutoff } },
        })
      ).count;
    } catch {
      usePrisma = false;
  ...
  for (const queue of allQueues) {
    try {
      for (const job of await queue.getActive()) {
        if (
          (job.processedOn ?? 0) > 0 &&
          Date.now() - (job.processedOn ?? 0) > STALE_LOCK_AGE_MINUTES * 60000
        ) {
          try {
            await job.moveToFailed(new Error('Stale lock'), 'cleanup-worker');
            released++;
          } catch {}
        }
      }
...
async function processCleanup(job: Job<CleanupJobData>): Promise<CleanupMetrics> {
  ...
  console.log(
    '[cleanup] Done - expired=' + ej + ' orphaned=' + of_ + ' stale=' + sl + ' ms=' + dur,
  );
  return {
    expired_jobs_removed: ej,
    orphaned_files_removed: of_,
    stale_locks_released: sl,
    duration_ms: dur,
...
export function createCleanupWorker(concurrency = 1): Worker<CleanupJobData, CleanupMetrics> {
  const w = new Worker<CleanupJobData, CleanupMetrics>('cleanup', processCleanup, {
...
export async function startCleanupScheduler(): Promise<NodeJS.Timeout> {
  const q = new Queue('cleanup', {
    connection: redisConnection,
    defaultJobOptions: {
      attempts: 1,
      removeOnComplete: { age: 86400, count: 24 },
      removeOnFail: { age: 86400, count: 48 },
    },
  });
