...
import { governanceStageSchema, governanceStatusSchema } from '@animaforge/events';
import {
  StageDefinition,
  GenerationResult,
  calculateProgress,
...
import { runGovernancePipeline, type GovernancePipelineJob } from './governancePipeline.js';
...
export type GenerationType = 'video' | 'audio' | 'avatar' | 'style_clone' | 'img_to_cartoon';
...
async function submitToAiApi(type: GenerationType, data: GenerationJobData) {
  ...
  switch (type) {
    case 'video':
      return submitVideoGeneration({ prompt: (data.params.prompt as string) ?? '', ...base });
    case 'audio':
      return submitAudioGeneration({ prompt: (data.params.prompt as string) ?? '', ...base });
    case 'avatar':
    ...
    case 'img_to_cartoon':
      return submitCartoonConversion({ imageUrl: (data.params.imageUrl as string) ?? '', ...base });
  }
}
...
async function pollAiJobUntilDone(
  ...
  for (let attempt = 0; attempt < AI_POLL_MAX_ATTEMPTS; attempt++) {
    ...
    const combinedProgress = Math.round(
      baseProgress + (stageWeight / totalWeight) * aiStatus.progress,
    );
    ...
    await publishEvent('generation.progress', {
      jobId: String(bullJob.id),
      userId: bullJob.data.user_id,
      stage: 'ai_processing',
      progress: Math.min(combinedProgress, 99),
      aiStatus: aiStatus.status,
    });
  ...
  try {
    ...
    if (cachedResult) {
      ...
      await emitRealtimeEvent(user_id, 'job:completed', {
        jobId,
        outputUrl: cachedResult.output_url,
        qualityScores: cachedResult.quality_scores,
        deduplicated: true,
      });
    ...
    await emitRealtimeEvent(user_id, 'job:started', {
      ...
      gpuEstimate: estimateGPU(type, tier),
    });
    await publishEvent('generation.started', {
      jobId,
      userId: user_id,
      projectId: project_id,
      type,
    ...
    await createJobRecord({
      ...
      userId: user_id,
      jobType: type,
      inputParams: { ...job.data.params, input_hash: inputHash },
      tier: tier ?? 'preview',
    });
    ...
    if (abortController.signal.aborted)
      throw new Error('Job cancelled: exceeded maximum duration for tier');
    ...
    await publishEvent('generation.ai_submitted', {
      jobId,
      userId: user_id,
      aiJobId: String(aiSubmission.jobId),
      estimatedDuration: aiSubmission.estimatedDuration,
    });
    ...
    const governanceResult = await runGovernancePipeline(governanceJob, async (stage, status) => {
      ...
      if (parsedStage.success && parsedStatus.success) {
        await publishEvent('governance.stage_changed', {
          jobId,
        ...
        console.warn(
          '[events] unrecognised governance stage/status, not published:',
          stage,
          status,
        );
      }
    ...
    if (!governanceResult.passed) {
      ...
      await publishEvent('governance.completed', {
        jobId,
        passed: false,
        blockedReason: governanceResult.blockedReason,
      });
      await publishEvent('generation.failed', {
        jobId,
        userId: user_id,
        reason: governanceResult.blockedReason ?? 'Governance blocked',
        stage: 'governance',
      });
    ...
    const qualityScores = {
      ...
      consistency: +(0.8 + Math.random() * 0.19).toFixed(3),
    };
    ...
    await publishEvent('governance.completed', {
      jobId,
      passed: true,
      manifest: governanceResult.manifest,
      watermarkId: governanceResult.watermarkId,
    });
    await publishEvent('generation.completed', {
      jobId,
    ...
    if (attemptsMade >= DLQ_MAX_RETRIES) {
      try {
        ...
        console.error(
          '[generation] Job ' + jobId + ' moved to DLQ after ' + attemptsMade + ' attempts',
        );
      } catch (dlqErr) {
        console.error(
          '[generation] Failed to enqueue to DLQ:',
...
export function createGenerationWorker(
  ...
  const worker = new Worker<GenerationJobData, GenerationResult>('generation', processGeneration, {
    connection: redisConnection,
    concurrency,
