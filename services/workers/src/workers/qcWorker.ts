...
const QC_THRESHOLDS = { lpips: 0.15, identityDrift: 0.1, lipSyncScore: 0.75, artifactScore: 0.9 };
...
async function emitRealtimeEvent(
  ...
  try {
    ...
    console.warn('[qc] Failed to emit:', err instanceof Error ? err.message : err);
  }
}
...
function runCheck(
  ...
  const borderline =
    !passed &&
    (lowerIsBetter
      ? score <= threshold + BORDERLINE_MARGIN
      : score >= threshold - BORDERLINE_MARGIN);
...
async function processQc(job: Job<QcJobData>): Promise<QcReport> {
  ...
  try {
    ...
    checks.push(
      runCheck('temporal_lpips', +(Math.random() * 0.25).toFixed(4), QC_THRESHOLDS.lpips, true),
    );
    ...
    checks.push(
      runCheck(
        'artifact_detection',
        +(0.7 + Math.random() * 0.3).toFixed(4),
        QC_THRESHOLDS.artifactScore,
        false,
      ),
    );
    ...
    const certificateId =
      allPassed && !hasBorderline ? 'qc-cert-' + crypto.randomUUID().slice(0, 12) : null;
...
export function createQcWorker(concurrency = 5): Worker<QcJobData, QcReport> {
  const worker = new Worker<QcJobData, QcReport>('qc', processQc, {
