...
export class ShotLockManager {
  ...
  lockShot(userId: string, shotId: string): LockInfo {
    ...
    if (ex && ex.userId !== userId && ex.expiresAt > Date.now())
  ...
  releaseAllForUser(userId: string): string[] {
    ...
    for (const [sid, lk] of this.locks.entries()) {
      if (lk.userId === userId) {
  ...
  getAllLocks(): Map<string, LockInfo> {
    ...
    for (const [sid, lk] of this.locks.entries()) {
      if (lk.expiresAt > Date.now())
        r.set(sid, { locked: true, lockedBy: lk.userId, expiresAt: lk.expiresAt });
    }
    return r;
  }
}
