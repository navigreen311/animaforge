...
export type NotificationType = 'email' | 'push' | 'in_app' | 'webhook';
...
export function getTemplate(type: EmailTemplateType, vars: Record<string, string> = {}): string {
...
export function generateWeeklyDigest(
  ...
  const weekNotifications = userNotifications.filter((n) => new Date(n.createdAt) >= oneWeekAgo);
