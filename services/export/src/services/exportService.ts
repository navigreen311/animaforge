...
let usePrisma = true;
async function withFallback<T>(p: () => Promise<T>, m: () => T): Promise<T> {
  if (!usePrisma) return m();
  try {
    return await p();
  } catch {
    usePrisma = false;
...
export function batchExport(
  ...
  const batchId = uuidv4(),
    cs = options.chunkSize ?? 10,
    codec = options.codec ?? 'h264',
    res = options.resolution ?? '1080p',
    fps = options.fps ?? 30,
    ej: ExportJob[] = [];
  for (let i = 0; i < shotIds.length; i += cs) {
    const ch = shotIds.slice(i, i + cs);
    ej.push(
      processVideoExport(
        createExportJob('video', {
          projectId,
          shotIds: ch,
          format,
          codec: codec as 'h264' | 'h265' | 'av1' | 'vp9',
          resolution: res,
          fps,
        }),
      ),
...
const EXPORT_PRESETS: ExportPreset[] = [
  {
    ...
    resolution: '1080p',
    fps: 30,
    bitrate_hint: '10000 kbps',
    description: 'Best for web playback',
  },
  {
    ...
    resolution: '4k',
    fps: 24,
    bitrate_hint: '18000 kbps',
    description: 'Professional broadcast standard',
  },
  {
    name: 'archive',
    label: 'Archive',
    ...
    resolution: '4k',
    fps: 60,
    bitrate_hint: '35000 kbps',
    description: 'Maximum quality archival',
  },
  {
    name: 'mobile',
    label: 'Mobile',
  ...
  },
  {
    ...
    resolution: '1080p',
    fps: 30,
    bitrate_hint: '8000 kbps',
    description: 'Tuned for social platforms',
  },
];
export function getPresets(): ExportPreset[] {
  return EXPORT_PRESETS;
}
export function getPresetByName(n: string): ExportPreset | undefined {
  return EXPORT_PRESETS.find((p) => p.name === n);
}
...
export function addWatermark(exportId: string, wc: WatermarkConfig) {
  ...
  if (j.status !== 'completed') throw new Error('Incomplete');
  ...
  return {
    ...
    config: {
      ...wc,
      opacity: Math.max(0, Math.min(1, wc.opacity)),
      scale: Math.max(0.05, Math.min(0.5, wc.scale)),
    },
    outputUrl: j.outputUrl!.replace('/exports/', '/exports/watermarked/'),
...
export function embedMetadata(exportId: string, md: MetadataPayload) {
  ...
  if (md.title) f['dc:title'] = md.title;
  if (md.artist) f['dc:creator'] = md.artist;
  if (md.description) f['dc:description'] = md.description;
  if (md.copyright) f['dc:rights'] = md.copyright;
  if (md.tags) f['dc:subject'] = md.tags.join(', ');
  return {
    exportId,
    embedded: true,
    format: (j.type === 'video' || j.type === 'audio' ? 'xmp' : 'exif') as 'exif' | 'xmp',
    fields: f,
...
export function getExportProgress(jobId: string) {
  ...
  if (!j) throw new Error('Not found');
  const el = Date.now() - new Date(j.createdAt).getTime();
  let eta: string | null = null;
  if (j.progress > 0 && j.progress < 100)
    eta = new Date(Date.now() + (el / j.progress) * 100 - el).toISOString();
  return {
    jobId,
    stage: j.status === 'completed' ? 'done' : j.status,
    percent: j.progress,
    eta,
  ...
  for (const j of jobs.values()) {
    ...
    if (p.projectId === projectId)
      e.push({
        ...
        type: j.type,
        status: j.status,
        format: (p.format as string) ?? 'unknown',
        fileSize: j.fileSize,
...
export async function deleteExport(exportId: string) {
  ...
  return { deleted: true, id: exportId };
}

export function estimateExportCost(p: {
  type: ExportJob['type'];
  format: string;
  shotCount: number;
  resolution?: string;
  ...
  switch (p.type) {
    case 'video': {
      const br = getBitrate(p.codec ?? 'h264', p.resolution ?? '1080p');
      sz = Math.round((br * p.shotCount * 5) / 8) * 1000;
      ec = Math.ceil(p.shotCount * (p.resolution === '4k' ? 3 : 1));
      dur = Math.ceil(p.shotCount * (p.resolution === '4k' ? 8 : 3));
      break;
    }
    case 'audio': {
      const m: Record<string, number> = { wav: 1411, mp3: 320, aac: 256, opus: 128 };
      sz = Math.round(((m[p.format] ?? 256) * p.shotCount * 5) / 8) * 1000;
      ec = Math.ceil(p.shotCount * 0.5);
      dur = Math.ceil(p.shotCount);
      break;
    }
    case 'project': {
      sz = 50000000 + (p.includeAssets ? 200000000 : 0);
      ec = p.includeAssets ? 5 : 2;
      dur = p.includeAssets ? 30 : 10;
      break;
    }
    case 'avatar': {
      const m: Record<string, number> = { gltf: 15e6, usd: 20e6, fbx: 25e6, bvh: 5e5, arkit: 8e6 };
      sz = m[p.format] ?? 10e6;
      ec = 2;
      dur = 5;
      break;
  ...
  const sc = Math.ceil(sz / (100 * 1024 * 1024)),
    cc = Math.ceil(sz / (500 * 1024 * 1024));
  let sl: string;
  if (sz >= 1e9) sl = (sz / 1e9).toFixed(1) + ' GB';
  else if (sz >= 1e6) sl = (sz / 1e6).toFixed(1) + ' MB';
  else sl = (sz / 1e3).toFixed(1) + ' KB';
  return {
    size_estimate_bytes: sz,
    size_estimate_label: sl,
    credits: ec + sc + cc,
    duration_estimate_seconds: dur,
    breakdown: { encoding_credits: ec, storage_credits: sc, cdn_credits: cc },
  };
}
