...
function createApp() {
  ...
  app.post('/billing/stripe-webhook', stripeWebhookMiddleware, (req: StripeWebhookRequest, res) => {
    if (!req.stripeEvent) {
...
describe('Stripe Billing Integration', () => {
  ...
  describe('POST /billing/checkout', () => {
    ...
    it('should reject when tier is missing', async () => {
      const res = await request(app).post('/billing/checkout').send({ userId: 'user-1' });
  ...
  describe('POST /billing/portal', () => {
    it('should create a portal session for an existing customer', async () => {
      ...
      const res = await request(app).post('/billing/portal').send({ userId: 'user-portal' });
    ...
    it('should return 404 when customer does not exist', async () => {
      const res = await request(app).post('/billing/portal').send({ userId: 'nonexistent-user' });
  ...
  describe('Webhook: customer.subscription.updated', () => {
    it('should update subscription tier', async () => {
      ...
      const customer = stripeService.createCustomer('user-update', 'update@animaforge.app');
  ...
  describe('Webhook: customer.subscription.deleted', () => {
    it('should cancel the subscription', async () => {
      const customer = stripeService.createCustomer('user-cancel', 'cancel@animaforge.app');
  ...
  describe('Webhook: invoice.payment_failed', () => {
    it('should mark subscription as past_due', async () => {
      const customer = stripeService.createCustomer('user-fail', 'fail@animaforge.app');
  ...
  describe('Credit allocation', () => {
    it('should allocate correct credits when subscription is created via checkout', async () => {
      ...
      const credits = (await import('../services/billingService')).getCredits('user-credits');
  ...
  describe('stripeService unit tests', () => {
    it('should create and retrieve customer by userId', () => {
      const customer = stripeService.createCustomer('user-direct', 'direct@animaforge.app');
    ...
    it('should throw on unknown tier for createSubscription', () => {
      const customer = stripeService.createCustomer('user-bad-tier', 'bad@animaforge.app');
    ...
    it('should handle invoice.payment_succeeded by topping up credits', async () => {
      const customer = stripeService.createCustomer('user-invoice', 'invoice@animaforge.app');
      ...
      const credits = (await import('../services/billingService')).getCredits('user-invoice');
