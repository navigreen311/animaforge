...
export const CreateContractSchema = z.object({
  ...
  terms: z.object({
    ...
    milestones: z
      .array(z.object({ name: z.string(), dueDate: z.string(), amount: z.number().min(0) }))
      .optional(),
...
export interface Contract {
  ...
  terms: {
    deliverables: string[];
    milestones?: { name: string; dueDate: string; amount: number }[];
    paymentTerms: string;
...
async function isPrismaAvailable(): Promise<boolean> {
  ...
  try {
    ...
    console.warn('[talent] Prisma unavailable -- falling back to in-memory store');
    ...
    try {
      ...
      if (db) {
        ...
        return {
          ...
          portfolio: db.portfolio as string[],
          ...
          averageRating:
            typeof db.averageRating === 'number' ? db.averageRating : Number(db.averageRating),
          reviewCount: db.reviewCount ?? 0,
          createdAt:
            db.createdAt instanceof Date ? db.createdAt.toISOString() : String(db.createdAt),
        };
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
...
export async function searchTalent(query: z.infer<typeof SearchTalentSchema>): Promise<{
  ...
  if (query.minRate !== undefined)
    results = results.filter(
      (p) => (p.rates.hourly ?? p.rates.daily ?? p.rates.project ?? 0) >= query.minRate!,
    );
  if (query.maxRate !== undefined)
    results = results.filter(
      (p) => (p.rates.hourly ?? p.rates.daily ?? p.rates.project ?? 0) <= query.maxRate!,
    );
  const total = results.length;
  const start = (query.page - 1) * query.limit;
  return {
    profiles: results.slice(start, start + query.limit),
    total,
    page: query.page,
    limit: query.limit,
  };
...
export async function createBooking(
  ...
  if (await isPrismaAvailable()) {
    try {
      ...
      console.warn(
        '[talent] Prisma createBooking failed:',
...
export async function updateBookingStatus(
  ...
  if (status === 'accepted') {
    const t = profiles.get(booking.talentId);
    if (t) t.availability = 'busy';
  }
  if (status === 'completed' || status === 'declined') {
    const t = profiles.get(booking.talentId);
    if (t) t.availability = 'available';
  }
  if (await isPrismaAvailable()) {
    try {
      await requirePrisma().talentBooking.update({ where: { id: bookingId }, data: { status } });
    } catch (err: unknown) {
      console.warn(
        '[talent] Prisma updateBookingStatus failed:',
        err instanceof Error ? err.message : String(err),
      );
    }
  }
...
export function reviewTalent(
  ...
  if (talentReviews.some((r) => r.reviewerId === data.reviewerId))
    return { error: 'Already reviewed this talent' };
...
export function matchTalent(requirements: {
  skills: string[];
  minRating?: number;
  availability?: 'available' | 'busy' | 'unavailable';
  maxRate?: number;
  ...
  candidates = requirements.availability
    ? candidates.filter((p) => p.availability === requirements.availability)
    : candidates.filter((p) => p.availability === 'available');
  if (requirements.minRating !== undefined)
    candidates = candidates.filter((p) => p.averageRating >= requirements.minRating!);
  if (requirements.maxRate !== undefined)
    candidates = candidates.filter(
      (p) => (p.rates.hourly ?? p.rates.daily ?? p.rates.project ?? 0) <= requirements.maxRate!,
    );

  const scored = candidates.map((p) => {
    const ts = p.skills.map((s) => s.toLowerCase());
    const mc = requiredSkills.filter((req) =>
      ts.some((t) => t.includes(req) || req.includes(t)),
    ).length;
  ...
  return scored
    ...
    .sort(
      (a, b) => b.matchRatio - a.matchRatio || b.profile.averageRating - a.profile.averageRating,
    )
    .map((s) => s.profile);
}
...
export function createContract(
  ...
  const booking = bookings.get(bookingId);
  if (!booking) return { error: 'Booking not found' };
  if (Array.from(contracts.values()).find((c) => c.bookingId === bookingId))
    return { error: 'Contract already exists for this booking' };
...
export function getBookingContract(bookingId: string): Contract | undefined {
...
export function addPortfolioItem(
  talentId: string,
  item: z.infer<typeof PortfolioItemSchema>,
...
export function getPortfolio(talentId: string): PortfolioItem[] {
  return portfolioItems.get(talentId) ?? [];
}
...
export function setAvailability(
  ...
  dates: string[],
  status: 'available' | 'busy' | 'unavailable',
): AvailabilityEntry[] | { error: string } {
  if (!profiles.has(talentId)) return { error: 'Talent not found' };
...
export function getTalentRating(talentId: string): TalentRatingResult | { error: string } {
  ...
  for (let i = 0; i < sorted.length; i++) {
    const w = i + 1;
    wSum += sorted[i].rating * w;
    wTotal += w;
  }
  return {
    talentId,
    weightedAverage: wTotal > 0 ? +(wSum / wTotal).toFixed(2) : 0,
    totalReviews: tr.length,
    breakdown,
  };
}
